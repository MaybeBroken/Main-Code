...
class _firing:
    def addLaser(data):
        ...
        Thread(target=fade.fadeOutGuiElement_ThreadedOnly, args=[modelNode, 25, "after", destroyNode, (modelNode)], daemon=True).start()
...
class lasers:
    ...
    def fire(self: None = _self, origin=None, target=None, destroy = True):
        ...
        if destroy:
            ...
            Thread(target=fade.fadeOutGuiElement_ThreadedOnly, args=[target, 100, None, None, None], daemon=True).start()

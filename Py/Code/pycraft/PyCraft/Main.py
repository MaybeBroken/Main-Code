...
from panda3d.core import TransparencyAttrib, Texture,DirectionalLight, AmbientLight, loadPrcFile, ConfigVariableString, AudioSound
from panda3d.core import WindowProperties, NodePath, TextNode, CullFaceAttrib, Spotlight, PerspectiveLens, SphereLight, PointLight, Point3, OccluderNode
from panda3d.core import CollisionTraverser, CollisionNode, CollisionBox, CollisionSphere, CollisionRay, CollisionHandlerQueue, Vec3, CollisionHandlerPusher
...
if Worldvars.winMode == 'full':
    ConfigVariableString('win-size', str(Worldvars.resolution[0]) +' ' +str(Worldvars.resolution[1])).setValue(str(Worldvars.resolution[0]) +' ' +str(Worldvars.resolution[1]))
    ConfigVariableString('fullscreen', 'true').setValue('true')
    ConfigVariableString('undecorated', 'true').setValue('true')

if Worldvars.winMode == 'win':
    ConfigVariableString('win-size', str(int(monitor[0].width/2)) +' ' +str(int(monitor[0].height/2))).setValue(str(int(monitor[0].width/2)) +' ' +str(int(monitor[0].height/2)))
    ConfigVariableString('fullscreen', 'false').setValue('false')
...
class Main(ShowBase):
        ...
        for root, dirs, files in os.walk('PyCraft/src/audio'):
            for name in files:
                if name.endswith((".wav", ".mp3", ".ogg")):
                    self.songARR.append(self.loader.loadSfx('PyCraft/src/audio/' +name))
    ...
    def newWorld(self):
        ...
        self.worldname = DirectEntry(pos = (-0.38, 0, 0.2),
        ...
        loadButton = DirectButton(text = "Create New World",
    ...
    def openWorld(self):
        ...
        self.worldListDisplay = DirectOptionMenu(parent=self.openWorldScreen,
                                                items = worldlist,
                                                scale = (.1, .2, .2),
                                                pos = (-0.75, 0, 0.5),
                                                text_font = self.font,
        ...
        loadButton = DirectButton(text = "Open World",
        ...
        delButton = DirectButton(text = "Delete World",
    ...
    def saveSettingChanges(self):
        ...
        saveData = [Worldvars.difficulty, Worldvars.font, Worldvars.winMode, Worldvars.resolution, Worldvars.reachDistance, Worldvars.renderDist, Worldvars.audioVolume]
    ...
    def changeRenderDist(self):
        ...
        self.renderDistance.configure(text = "Render Distance: " +str(int(Worldvars.renderDist)))
    
    def changeAudioVolume(self):
        Worldvars.audioVolume = int(self.audioVolume.cget('value'))
        self.audioVolume.configure(text = "Master Volume: " +str(int(Worldvars.audioVolume)))
        self.music.setVolume(Worldvars.audioVolume / 100)
    ...
    def settings(self):
        ...
        self.difficultyOption = DirectButton(text = "Difficulty: " +Worldvars.difficulty,
        ...
        self.fontOption = DirectButton(text = "Font: " +Worldvars.font,
        ...
        GraphicsOptions = DirectButton(text = "Graphics Options:",
        ...
        saveChanges = DirectButton(text = "Save Changes",
    ...
    def GraphicsOptionsMGR(self):
        ...
        self.changeWinMode = DirectButton(text = "Mode: " +Worldvars.winMode,
        ...
        self.changeRes = DirectButton(text = "Resolution: " +str(Worldvars.resolution),
        ...
        saveGChanges = DirectButton(text = "Save Changes",
    ...
    def loadWorld(self, name):
        ...
        for blank in worldBlockArr:
            self.createNewBlock(x=(worldBlockArr[i][0]), y=(worldBlockArr[i][1]), z=(worldBlockArr[i][2]), type=worldBlockArr[i][3])
            i=i+1
    ...
    def saveWorld(self):
        ...
        array = Parser.saveWorld.worldToText(Parser.saveWorld, worldARR=Worldvars.allBlocks)
        Fmgr.openWorldFile.saveWorld(Fmgr.openWorldFile, worldARR=array, name=self.worldnameFile)
    ...
    def worldOptionsScreen(self):
        ...
        btn = DirectButton(text = "New world",
        ...
        btn = DirectButton(text = "Open World",
        ...
        btn = DirectButton(text = "Settings",
    ...
    def launch(self):
        ...
        self.setupCamera()
        self.setupControls()
        Ui.bar.buildHotbar(self=Ui.bar, main=self, textureAlias=Texture.FTNearest, Sprites=self.sprites, font=self.font, TextNode=TextNode, aspect2d=self.aspect2d)
        thread.Thread(target=self.renderBlocks, name='render',daemon=True).start()
    ...
    def menu(self):
        ...
        title = DirectLabel(text = "Minecraft",
                            scale = 0.125,
                            pos = (0, 0, 0.75),
                            ...
                            text_font = self.font,
                            text_fg = (1, 1, 1, 1))
        title2 = DirectLabel(text = "recreated in python by",
                            scale = 0.07,
                            pos = (0, 0, 0.6),
                            parent = self.titleMenu,
                            relief = None,
                            text_font = self.font,
                            text_fg = (1, 1, 1, 1))
        title3 = DirectLabel(text = "MaybeBroken",
                            scale = 0.1,
                            pos = (0, 0, 0.45),
                            parent = self.titleMenu,
                            relief = None,
    ...
    def death(self):
        ...
        label = DirectLabel(text = "Game Over!",
                    ...
                    pos = (0, 0, 0.2),
                    text_font = self.font)
        self.finalScoreLabel = DirectLabel(text = "You Died",
                                   parent = self.gameOverScreen,
                                   scale = 0.15,
                                   pos = (0, 0, 0),
                                   text_font = self.font)
        
        btn = DirectButton(text = "Main Menu",
                        command = restartGame,
                        extraArgs = [self],
                        pos = (-0.3, 0, -0.2),
                        parent = self.gameOverScreen,
    ...
    def escape(self):
        if Worldvars.hp > 0:
            ...
            btn = DirectButton(text = "Save World",
    ...
    def update(self, task):
        ...
        if Worldvars.inMenu ==True:
            ...
            thread.Thread(target=self.updateItemsWorld, name='updateItemsWorld').run()
    ...
    def openInventory(self):
        Ui.HUD.drawInventoryFrames(self=Ui.HUD, mainApp=self, textureAlias=Texture.FTNearest, GeomSprites=self.GeomSprites, font=self.font, TextNode=TextNode, aspect2d=self.aspect2d)
        ...
        try:
            if self.rayQueue.getNumEntries() > 1 and Worldvars.inMenu != True:
                ...
                if distanceFromPlayer <= Worldvars.reachDistance:
                    ...
                    thread.Thread(target=self.removeBlockAnimation, args=[hitObject, hitObjectName, hitNodePath], daemon=True).start()
        except: None
    
    def removeBlockAnimation(self, hitObject, hitObjectName, hitNodePath):
        for i in range(round(Worldvars.kinds_of_blocks[utils.item_to_int(hitObjectName[3])][1] * 1000)):
            if self.keyMap['primary']:
                t.sleep(0.001)
            else:
                t.sleep(0.1)
        ...
        if self.keyMap['primary']:
            ...
            self.createNewdroppedItem(hitObjectName[0], hitObjectName[1], hitObjectName[2], hitObjectName[3])
            ...
            if self.rayQueue.getNumEntries() > 1 and Worldvars.inMenu != True:
                ...
                if Ui.Inventory.removeOneItem_hotbar(Ui.Inventory) == True:
                    if distanceFromPlayer <= Worldvars.reachDistance:
                        ...
                        self.createNewBlock(int(newBlockPos.x), int(newBlockPos.y), int(newBlockPos.z), self.selectedBlockType)
        except: None
    ...
    def setupCamera(self):
        ...
        Ui.ShaderCall.setupShaders(self=Ui.ShaderCall, mainApp=self, light = self.SceneLightNode)
    ...
    def generateTerrain(self):
        ...
        for index1 in range(round(camPosX - (Worldvars.renderDist/2)), round(camPosX + (Worldvars.renderDist/2))):
            for index2 in range(round(camPosY - (Worldvars.renderDist/2)), round(camPosY + (Worldvars.renderDist/2))):
                for index3 in range(Worldvars.worldz):
                    index3 = gen.terrainGen.makeNoiseMap(gen.terrainGen, index1/Worldvars.dens, index2/Worldvars.dens, index3/Worldvars.dens, Worldvars.seed)
                    if any(item == [index1, index2, index3, (I[0] for I in Worldvars.allItems)] for item in Worldvars.allBlocks):
                        None
                    else: self.createNewBlock(index1*2, index2*2, index3*2, 'stone')
        ...
        for root, dirs, files in os.walk('PyCraft/src/models/'):
            for name in files:
                if name.endswith((".egg")):
                    ...
                    self.__setattr__(file_no_extension +'Block', self.loader.loadModel('PyCraft/src/models/' +file_no_extension +'/' +name))
            ...
            try:
                for node in self.itemRenderNode.getChildren():
                    ...
                    if node.getDistance(self.camera) <= 4:
                        ...
                        hitObjectName[1] = int(hitObjectName[1])
                        hitObjectName[2] = int(hitObjectName[2])
                        if Ui.Inventory.addItem(self=Ui.Inventory, item=hitObjectName[3], amount=1) == True:
                            node.removeNode()
            except: None

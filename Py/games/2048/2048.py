...
import random
import time
from win32api import GetCursorPos, SetCursorPos, GetSystemMetrics
...
def load_subclass():
    ...
    winh = GetSystemMetrics(0)
    winv = GetSystemMetrics(1)
...
class mainGame(ShowBase):
    ...
    def gameWindow(self):
        ...
        self.highscore = OnscreenText(parent=self.frame, pos = (-0.9, .9, 0.9), text = 'High score: 0', scale = 0.08, fg = textColor)
        self.currentscore = OnscreenText(parent=self.frame, pos = (0.9, .9, 0.9), text = 'Current Score: 0', scale = 0.08, fg = textColor)

        icon1 = DirectFrame(parent=self.render2d, frameSize = (-1, -.5, -1, -.5), frameColor = gameGridColor, image=self.frametexture, image_pos = (-.75, 0, -.75), image_scale = (.25, .25, .25))
        icon2 = DirectFrame(parent=self.render2d, frameSize = (-.5, 0, -1, -.5), frameColor = gameGridColor, image=self.frametexture, image_pos = (-.25, 0, -.75), image_scale = (.25, .25, .25))
        icon3 = DirectFrame(parent=self.render2d, frameSize = (0, .5, -1, -.5), frameColor = gameGridColor, image=self.frametexture, image_pos = (.25, 0, -.75), image_scale = (.25, .25, .25))
        icon4 = DirectFrame(parent=self.render2d, frameSize = (.5, 1, -1, -.5), frameColor = gameGridColor, image=self.frametexture, image_pos = (.75, 0, -.75), image_scale = (.25, .25, .25))

        icon5 = DirectFrame(parent=self.render2d, frameSize = (-1, -.5, -.5, 0), frameColor = gameGridColor, image=self.frametexture, image_pos = (-.75, 0, -.25), image_scale = (.25, .25, .25))
        icon6 = DirectFrame(parent=self.render2d, frameSize = (-.5, 0, -.5, 0), frameColor = gameGridColor, image=self.frametexture, image_pos = (-.25, 0, -.25), image_scale = (.25, .25, .25))
        icon7 = DirectFrame(parent=self.render2d, frameSize = (0, .5, -.5, 0), frameColor = gameGridColor, image=self.frametexture, image_pos = (.25, 0, -.25), image_scale = (.25, .25, .25))
        icon8 = DirectFrame(parent=self.render2d, frameSize = (.5, 1, -.5, 0), frameColor = gameGridColor, image=self.frametexture, image_pos = (.75, 0, -.25), image_scale = (.25, .25, .25))

        icon9 = DirectFrame(parent=self.render2d, frameSize = (-1, -.5, 0, .5), frameColor = gameGridColor, image=self.frametexture, image_pos = (-.75, 0, .25), image_scale = (.25, .25, .25))
        icon10 = DirectFrame(parent=self.render2d, frameSize = (-.5, 0, 0, .5), frameColor = gameGridColor, image=self.frametexture, image_pos = (-.25, 0, .25), image_scale = (.25, .25, .25))
        icon11 = DirectFrame(parent=self.render2d, frameSize = (0, .5, 0, .5), frameColor = gameGridColor, image=self.frametexture, image_pos = (.25, 0, .25), image_scale = (.25, .25, .25))
        icon12 = DirectFrame(parent=self.render2d, frameSize = (.5, 1, 0, .5), frameColor = gameGridColor, image=self.frametexture, image_pos = (.75, 0, .25), image_scale = (.25, .25, .25))

        icon13 = DirectFrame(parent=self.render2d, frameSize = (-1, -.5, .5, 1), frameColor = gameGridColor, image=self.frametexture, image_pos = (-.75, 0, .75), image_scale = (.25, .25, .25))
        icon14 = DirectFrame(parent=self.render2d, frameSize = (-.5, 0, .5, 1), frameColor = gameGridColor, image=self.frametexture, image_pos = (-.25, 0, .75), image_scale = (.25, .25, .25))
        icon15 = DirectFrame(parent=self.render2d, frameSize = (0, .5, .5, 1), frameColor = gameGridColor, image=self.frametexture, image_pos = (.25, 0, .75), image_scale = (.25, .25, .25))
        icon16 = DirectFrame(parent=self.render2d, frameSize = (.5, 1, .5, 1), frameColor = gameGridColor, image=self.frametexture, image_pos = (.75, 0, .75), image_scale = (.25, .25, .25))

        textframe1 = OnscreenText('-', pos = (-1.36, -.8), scale = .25)
        textframe2 = OnscreenText('-', pos = (-.45, -.8), scale = .25)
        textframe3 = OnscreenText('-', pos = (.45, -.8), scale = .25)
        textframe4 = OnscreenText('-', pos = (1.36, -.8), scale = .25)

        textframe5 = OnscreenText('-', pos = (-1.36, -.25), scale = .25)
        textframe6 = OnscreenText('-', pos = (-.45, -.25), scale = .25)
        textframe7 = OnscreenText('-', pos = (.45, -.25), scale = .25)
        textframe8 = OnscreenText('-', pos = (1.36, -.25), scale = .25)

        textframe9 = OnscreenText('-', pos = (-1.36, .25), scale = .25)
        textframe10 = OnscreenText('-', pos = (-.45, .25), scale = .25)
        textframe11 = OnscreenText('-', pos = (.45, .25), scale = .25)
        textframe12 = OnscreenText('-', pos = (1.36, .25), scale = .25)

        textframe13 = OnscreenText('-', pos = (-1.36, .72), scale = .25)
        textframe14 = OnscreenText('-', pos = (-.45, .72), scale = .25)
        textframe15 = OnscreenText('-', pos = (.45, .72), scale = .25)
        textframe16 = OnscreenText('-', pos = (1.36, .72), scale = .25)
        ...
        th.Thread(None, self.updatecrosshair, 'Update pointer crosshair', daemon=True).start()
    ...
    def updateScore(self):
        while 1==1:
            ...
            try: self.currentscore.configure(text = 'Current Score: ' +str(Vars.score))
            except: print("failed to update score")
            try: self.highscore.configure(text = 'High Score: ' +str(Vars.highScore))
            except: print("failed to update high score")
            time.sleep(.1)
    ...
    def updateValues(self, up:bool = False, down:bool = False, left:bool = False, right:bool = False ):
        if up==True:
            print("Up")
            self.mathUp()
        if down==True:
    ...
    def mathRight(self):
        for i in range(2):
            for row in range(len(Vars.gameGrid)):
                ...
                if Vars.gameGrid[row][2] == Vars.gameGrid[row][3] and Vars.gameGrid[row][3] != 0:
                    Vars.gameGrid[row][3] = Vars.gameGrid[row][2]*2
                    Vars.gameGrid[row][2] = Vars.gameGrid[row][1]
                    Vars.gameGrid[row][1] = Vars.gameGrid[row][0]
                    Vars.gameGrid[row][0] = 0
                elif Vars.gameGrid[row][1] == Vars.gameGrid[row][2] and Vars.gameGrid[row][2] != 0:
                    Vars.gameGrid[row][2] = Vars.gameGrid[row][1]*2
                    Vars.gameGrid[row][1] = Vars.gameGrid[row][0]
                    Vars.gameGrid[row][0] = 0
                elif Vars.gameGrid[row][0] == Vars.gameGrid[row][1] and Vars.gameGrid[row][1] != 0:
    ...
    def mathLeft(self):
        for i in range(2):
            for row in range(len(Vars.gameGrid)):
                ...
                if Vars.gameGrid[row][1] == Vars.gameGrid[row][0] and Vars.gameGrid[row][1] != 0:
                    Vars.gameGrid[row][0] = Vars.gameGrid[row][1]*2
                    Vars.gameGrid[row][1] = Vars.gameGrid[row][2]
                    Vars.gameGrid[row][2] = Vars.gameGrid[row][3]
                    Vars.gameGrid[row][3] = 0
                elif Vars.gameGrid[row][2] == Vars.gameGrid[row][1] and Vars.gameGrid[row][2] != 0:
                    Vars.gameGrid[row][1] = Vars.gameGrid[row][2]*2
                    Vars.gameGrid[row][2] = Vars.gameGrid[row][3]
                    Vars.gameGrid[row][3] = 0
                elif Vars.gameGrid[row][3] == Vars.gameGrid[row][2] and Vars.gameGrid[row][3] != 0:
    ...
    def mathUp(self):
        for i in range(2):
            for row in range(len(Vars.gameGrid)):
                ...
                if Vars.gameGrid[1][row] == Vars.gameGrid[0][row] and Vars.gameGrid[1][row] != 0:
                    Vars.gameGrid[0][row] = Vars.gameGrid[1][row]*2
                    Vars.gameGrid[1][row] = Vars.gameGrid[2][row]
                    Vars.gameGrid[2][row] = Vars.gameGrid[3][row]
                    Vars.gameGrid[3][row] = 0
                elif Vars.gameGrid[2][row] == Vars.gameGrid[1][row] and Vars.gameGrid[2][row] != 0:
                    Vars.gameGrid[1][row] = Vars.gameGrid[2][row]*2
                    Vars.gameGrid[2][row] = Vars.gameGrid[3][row]
                    Vars.gameGrid[3][row] = 0
                elif Vars.gameGrid[3][row] == Vars.gameGrid[2][row] and Vars.gameGrid[3][row] != 0:
    ...
    def mathDown(self):
        for i in range(2):
            for row in range(len(Vars.gameGrid)):
                ...
                if Vars.gameGrid[2][row] == Vars.gameGrid[3][row] and Vars.gameGrid[3][row] != 0:
                    Vars.gameGrid[3][row] = Vars.gameGrid[2][row]*2
                    Vars.gameGrid[2][row] = Vars.gameGrid[1][row]
                    Vars.gameGrid[1][row] = Vars.gameGrid[0][row]
                    Vars.gameGrid[0][row] = 0
                elif Vars.gameGrid[1][row] == Vars.gameGrid[2][row] and Vars.gameGrid[2][row] != 0:
                    Vars.gameGrid[2][row] = Vars.gameGrid[1][row]*2
                    Vars.gameGrid[1][row] = Vars.gameGrid[0][row]
                    Vars.gameGrid[0][row] = 0
                elif Vars.gameGrid[0][row] == Vars.gameGrid[1][row] and Vars.gameGrid[1][row] != 0:
                ...
                for index2 in range(len(Vars.gameGrid[index1])):
                    if Vars.gameGrid[index1][index2] * 1 == 0:
                        if index1 == randomindex1 and index2 == randomindex2:
                            ...
                            print("placed number in slot " +str(index1) +", " +str(index2) +':  randomly selected numbers were ' +str(randomindex1) +', ' +str(randomindex1))
                            randomActive = False
                            break
    ...
    def updateScreen(self):
        ...
        for i in range(len(Vars.gameGrid)):
            print('[' +str(int(time.monotonic()-start_time)) +"] --> " +str(Vars.gameGrid[i]))
